...
public sealed class StillGreenhousesServerSystem : ModSystem
{
    ...
    )
    {
        ...
        api.Logger.Notification(
            $"[StillGreenhouses] Server loaded v0.10.16a. " +
    ...
    )
    {
        ...
        // The client is subscribed before the scan. The first authoritative
        // SetChunkStateAndPublish() pushes the real result.
        PendingDiscoveryScans.TryAdd(
            chunkKey,
            0
        );
    }
    ...
    )
    {
        ...
        foreach (
            ...
            in PendingGreenhouseRevalidations.Keys
        )
        {
            if (
    ...
    )
    {
        ...
        foreach (string playerUid in subscribers.Keys)
        {
            if (
    ...
    )
    {
        ChunkDiscoveryResult discovery =
            DiscoverManagedRoomsInChunk(
                api,
                chunkKey
            );

        CommitChunkDiscovery(discovery);
    }

    private ChunkDiscoveryResult DiscoverManagedRoomsInChunk(
        ICoreServerAPI api,
        ChunkKey chunkKey
    )
    {
        ...
        if (registry == null)
        {
            return ChunkDiscoveryResult.Unavailable(
                chunkKey
            );
        }
        ...
        HashSet<RoomRevalidationKey> seenRoomKeys =
            new();
        ...
        List<RoomBounds> incompleteRoomBounds = new();

        bool[] coveredRoomPositions = new bool[
            StillGreenhousesShared.ChunkSize
            * StillGreenhousesShared.ChunkSize
            * StillGreenhousesShared.ChunkSize
        ];

        int discoveryAnchorPositions = 0;
        int roomRegistryQueries = 0;
        int coveredRoomPositionsSkipped = 0;
        int anchorRoomInstances = 0;
        int incompleteAnchorRooms = 0;
        int classifiedManagedRooms = 0;
        int viableManagedRooms = 0;
        int skippedTooSmallRooms = 0;
        int roomSizes1To2 = 0;
        int roomSizes3To6 = 0;
        int roomSizes7To15 = 0;
        int roomSizes16To31 = 0;
        int roomSizes32Plus = 0;
        ...
        )
        {
            ...
            )
            {
                ...
                )
                {
                    ...
                    if (coveredRoomPositions[localIndex])
                    {
        ...
        return new ChunkDiscoveryResult(
            ChunkKey: chunkKey,
            Loaded: true,
            Complete: incompleteAnchorRooms == 0,
            ManagedRooms: discoveredRooms,
            IncompleteRoomBounds: incompleteRoomBounds,
            DiscoveryAnchorPositions: discoveryAnchorPositions,
            RoomRegistryQueries: roomRegistryQueries,
            CoveredRoomPositionsSkipped: coveredRoomPositionsSkipped,
            AnchorRoomInstances: anchorRoomInstances,
            IncompleteAnchorRooms: incompleteAnchorRooms,
            ClassifiedManagedRooms: classifiedManagedRooms,
            ViableManagedRooms: viableManagedRooms,
            SkippedTooSmallRooms: skippedTooSmallRooms,
            RoomSizes1To2: roomSizes1To2,
            RoomSizes3To6: roomSizes3To6,
            RoomSizes7To15: roomSizes7To15,
            RoomSizes16To31: roomSizes16To31,
            RoomSizes32Plus: roomSizes32Plus
        );
    }
    ...
    )
    {
        ...
        return DiscoveryAnchorBlockIdentityCache
            .GetOrAdd(
                block.Id,
                _ => StillGreenhousesShared
    ...
    )
    {
        ...
        )
        {
            ...
            bool hasIndexedGreenhouse = false;
            ...
            if (
                !hasIndexedGreenhouse
                && ChunkSubscribers.ContainsKey(candidate)
            )
            {
    ...
    )
    {
        ...
        )
        {
            ...
            if (completeNegative)
            {
                // Preserve normal negative-cache invalidation. Region-level
                // former-room recovery below is independent of whether the
                // containing chunk is otherwise positive or negative.
    ...
    )
    {
        ...
        if (incompleteDiscoveryChunks > 0)
        {
            Interlocked.Increment(
    ...
    )
    {
        ...
        if (
            api == null
            || config?.Enabled != true
            || !ScheduledIncompleteChunkRetries.TryAdd(
                chunkKey,
                0
            )
        )
    ...
    )
    {
        ...
        {
            RemoveAllSubscriptionsForChunk(
                chunkKey
            );
        }
    }
    ...
    private void RemoveAllSubscriptionsForChunk(
        ChunkKey chunkKey
    )
    {
        if (
